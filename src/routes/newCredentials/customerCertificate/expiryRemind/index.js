...
function ExpiryRemind({
  ...
  const searchProps = {
    ...
    onSearch: (value) => {
      dispatch({
        type: 'newCustomerCertificate/getCertificateList',
        payload: {
          ...value,
        ...
        const handleClick = (index) => {
          ...
          if (index === 1) {
            if (orgCertificateType === '1') {
              url = 'newCustomerCertificate/getRegistDetaiList'
              modalTitle = '查看注册证'
              reqData = {
                certificateId,
                replacedFlag: false,
              ...
              url = 'newCustomerCertificate/getprodFactoryDetai'
              modalTitle = '查看厂家/总代证件'
              reqData = {
                factoryAgentCertificateId: certificateId,
              }
            } else if (orgCertificateType === '3') {
              url = 'newCustomerCertificate/getAuthDetail'
              modalTitle = '查看授权书'
              reqData = {
                certificateId,
              }
            } else if (orgCertificateType === '4') {
              url = 'newCustomerCertificate/getPowerDetail'
              modalTitle = '查看委托书'
              reqData = {
                certificateId,
              }
            } else if (orgCertificateType === '5') {
              url = 'newCustomerCertificate/getOtherDetail'
              modalTitle = `查看${orgCertificate}`
              reqData = {
                certificateId,
              }
            } else if (orgCertificateType === '6') {
              url = 'newCustomerCertificate/getCompanyDetail'
              modalTitle = '查看企业三证'
              reqData = {
                certificateId,
                orgId: supplierOrgId,
            ...
            dispatch({
              type: 'newCustomerCertificate/updateState',
              payload: {
                modalTitle,
              },
            })
            ...
            /** @desc 临时注释 提醒直接置为成功 */
            dispatch({
              type: 'newCustomerCertificate/getCertificateRemind',
              payload: {
                addBy,
                orgCertificate,
                supplierOrgId,
  ...
  return (
    <div key={tabIndex}>
      ...
      <Table
        ...
        loading={!!effects['newCustomerCertificate/getCertificateList']}
        rowKey="index"
      />
    </div>
  )
