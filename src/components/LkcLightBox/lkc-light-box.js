  ...
  render() {
    ...
    const retImageList = (data = []) => {
      ...
      data.map((item) => {
        ...
        retArr.push(
          <div
            key={item.index}
            className={`${styles['lkc-bottom-imgList-li']} ${
              photoIndex === item.index ? styles['lkc-bottom-image-selected'] : ''
            }`}
            onClick={() => {
              that.resetState()
    ...
    const imageList = (data, key) => {
      const retList = data.map((item) => {
        ...
        const list = image.map((i, index) => {
          const flag = () => {
            if (currentData && Object.keys(currentData).length) {
              if (i === currentData.value) {
                ...
                return styles['lkc-bottom-image-selected']
              }
              return ''
            }
            return ''
          ...
          return (
            <div key={i} className={`${styles['lkc-bottom-imgList-li']} ${flag()}`}>
              <Element name={i}>
                <a
                  id={i}
                  onClick={() => {
        ...
        return (
          <div key={text}>
            <h3 className={styles['lkc-second-title']}>{text}</h3>
            <div>{list}</div>
          </div>
        )
      })
    ...
    const getThumGroup = () => {
      ...
      const retGroup = data.map((items) => {
        ...
        return (
          <div key={key}>
            <h2 className={styles['lkc-first-title']}>{groupTitle}</h2>
            <div className={styles['lkc-imgList-main']}>{imageList(imgs, key)}</div>
          </div>
        )
      })
        ...
        {nextSrc && (
          ...
          {imageDetail && (
            <div
              ...
              className={styles['lkc-light-describe']}
          ...
          {/* 左侧描述信息 start */}
          {imageHeadDetail && (
            <div className={styles['lkc-light-head']} style={{ left: `${imageDetailWidth}px` }}>
              {imageHeadDetail}
            </div>
          )}
          ...
          {!this.props.dataSource && (
            <div
              className={`${styles['lkc-light-bottom']}`}
              style={imageDetail ? { left: imageDetailWidth } : { left: 0 }}
            >
              <div className={`${styles['lkc-bottom-imgList']}`}>
                <div className={`${styles['lkc-bottom-imgList-left']}`}>
                  {current === 1 ? (
                    ''
                  ) : (
                    <button
                      type="button"
                      className={`${styles['lkc-imgList-prev']}`}
                      key="prev"
                      aria-label={this.props.prevLabel}
                      onClick={() => {
                        this.scroll('left')
                ...
                <div className={`${styles['lkc-bottom-imgList-main']}`}>
                  <div style={scrollSyles}>{thumbnail}</div>
                </div>
                <div className={`${styles['lkc-bottom-imgList-right']}`}>
                  {current * 5 < imageArr.length ? (
                    <button // Move to next image button
                      type="button"
                      className={`${styles['lkc-imgList-next']}`}
                      key="next"
                      aria-label={this.props.nextLabel}
                      onClick={() => {
                        this.scroll('right')
          ...
          {/* 兼容于新版本 底部详情 start */}
          {this.props.dataSource && (
            <div className={styles['lkc-light-new-bottom']}>{getCertificateDetail()}</div>
          )}
          ...
          {this.props.dataSource && (
            <div
              id="containerElement"
              className={styles['lkc-light-new-right']}
              onWheel={(e) => {
                e.stopPropagation()
              }}
            >
