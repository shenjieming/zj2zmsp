...
const services = getServices({
  getCustomerListData: { url: '/certificate/register/push/supplier/list', type: 'post' },
  customerDetail: '/contacts/supplier/relation/finish/org', // 供应商详情
  getCustomerNo: '/certificate/customer/register/push/statistics',
  getReviewListData: '/certificate/customer/register/push/list/review', // 待审核证件列表
  getRefusedListData: '/certificate/customer/register/push/list/refused', // 已拒绝证件列表
...
export default modelExtend({
  namespace: 'newCertificateAudit',
  state: initState,
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname, search }) => {
        if (pathname === '/newCredentials/newCertificateAudit') {
  ...
  effects: {
    ...
    * getCustomerDetail({ payload }, { select, call, update }) {
      const { selectedCustomer } = yield select(({ newCertificateAudit }) => newCertificateAudit)
