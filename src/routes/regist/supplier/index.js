...
import { Form, Input, Button, Row, Col, Icon, Tooltip, Select, message, AutoComplete, Radio } from 'antd'
      ...
      >
        <FormItem {...FORM_ITEM_LAYOUT} label="供应商名称">
          {getFieldDecorator('orgName', {
            ...
            //   filterOption={false}
            // >
            //   {supplierList.map(item => (<Option key={item.supplierId} value={item.supplierName} >{item.supplierName}</Option>),
            //   )}
            // </AutoComplete>
            <Input placeholder="请输入营业执照上的名称" />,
          )}
        </FormItem>
        ...
        <FormItem {...FORM_ITEM_LAYOUT} label="设置密码">
          {getFieldDecorator('password', {
            ...
            <Input className="ant-input" onChange={() => { pasdChange() }} type="password" placeholder="请输入6-20位密码" />,
          )}
        </FormItem>
        <FormItem {...FORM_ITEM_LAYOUT} label="确认密码">
          {getFieldDecorator('confirmPassword', {
            ...
            <Input type="password" onChange={() => { confirmPasdChange() }} placeholder="请再次输入密码" />,
          )}
        </FormItem>
        <FormItem {...FORM_ITEM_LAYOUT} label="真实姓名">
          {getFieldDecorator('realName', {
            ...
            <Input placeholder="请输入您真实姓名，方便医院认识您" />,
          )}
        </FormItem>
