...
let idIndex = 0
class AekInputNumber extends React.Component {
  constructor(props) {
    super(props)
    ...
    idIndex += 1
    this.id = `aekInputNumber${idIndex}`
  }
  componentWillMount() {
    ...
    return (
      <span id={this.id}>
        <Tooltip
          ...
          title={title}
          placement="topLeft"
          getPopupContainer={() => document.getElementById(`aekInputNumber${idIndex}`)}
        >
          <InputNumber {...props} />
        </Tooltip>
      </span>
...
AekInputNumber.propTypes = propTypes
export default AekInputNumber
