...
const namespace = 'bankCreditDetail'
const propTypes = {
  bankCreditDetail: PropTypes.object,
  addressList: PropTypes.array,
  loading: PropTypes.object,
}
const InputMaxWidth = 400
const BankCreditDetail = ({ bankCreditDetail, loading, addressList }) => {
  const { getLoading } = getBasicFn({ namespace, loading })
  const { detail } = bankCreditDetail
...
BankCreditDetail.propTypes = propTypes
export default connect(({ bankCreditDetail, loading, app: { constants: { addressList } } }) => ({
  bankCreditDetail,
  loading,
  addressList,
}))(Form.create()(BankCreditDetail))
