...
const AddCertificateModal = ({
  ...
  function handleOk() {
    validateFields((errors, vals) => {
      ...
      dispatchAction({
        ...
        payload: {
          ...currentItem,
          supplierCertificateId: currentItem.certificateId,
          ...data,
        },
      })
    })
  ...
  const eventFun = {
    ...
    onStartTimeChange(filed) {
      filed &&
        dispatchAction({
          payload: {
            fileEndDate: moment(moment(filed).add(4, 'year'))
              .add(-1, 'days')
              .format('YYYY-MM-DD'),
          },
        })
  ...
  return (
    <Modal {...modalOpts}>
      <Spin spinning={getLoading('getAddRegistDetail', 'addRegist')}>
        <Form>
          <GetFormItem
            formData={form(initObj, eventFun, currentItem)}
          />
        </Form>
      </Spin>
    </Modal>
